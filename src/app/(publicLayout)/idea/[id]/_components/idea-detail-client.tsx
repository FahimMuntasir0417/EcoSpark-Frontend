...
function formatMetric(value: number | null | undefined, suffix = "", fallback = "N/A") {
...
function getCommentAuthorName(comment: Comment) {
  ...
  const candidates = [record.authorName, record.userName, author?.name, user?.name];
...
function isIdeaInteractiveStatus(_status?: string | null, _publishedAt?: string | null) {
  return true;
}
    ...
    if (isAxiosError(error)) {
      ...
      const backendMessage = Array.isArray(backendMessageValue)
        ...
        : typeof backendMessageValue === "string"
          ...
          : "";
      const endpointText =
        method && url ? ` Endpoint: ${method} ${url}.` : "";
...
function Badge({ children, tone = "neutral" }: { children: ReactNode; tone?: "neutral" | "accent" | "success" }) {
  return (
    <span
      className={cn(
...
function ActionCard({ title, description, icon: Icon, children }: { title: string; description: string; icon: LucideIcon; children: ReactNode }) {
  return (
  ...
  return (
    <div className="rounded-2xl border border-sky-100 bg-sky-50/70 p-4">
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        <Link href="/login" className="rounded-full bg-slate-950 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-slate-800">Login</Link>
        <Link href="/register" className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition-colors hover:border-slate-300 hover:text-slate-950">Create account</Link>
      </div>
    </div>
  );
}
function CommentThreadItem({
  ...
  const replies = (repliesQuery.data?.data ?? []).filter((reply) => reply.id !== comment.id);
  ...
  const onDelete = async () => {
    ...
    try {
      const response = await deleteCommentMutation.mutateAsync({ id: comment.id });
      onFeedback({
        type: "success",
  ...
  return (
    <article className={cn("rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm", depth > 0 && "bg-slate-50/70")}>
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            <p className="text-sm font-semibold text-slate-950">{getCommentAuthorName(comment)}</p>
            <Badge>{depth === 0 ? "Comment" : "Reply"}</Badge>
            {isOwner ? <Badge tone="accent">Your comment</Badge> : null}
          </div>
          <p className="text-xs uppercase tracking-[0.16em] text-slate-400">{getCommentTimestamp(comment)}</p>
        </div>

        <div className="flex flex-wrap gap-2">
          {isAuthenticated && canReply ? (
            <Button type="button" variant="outline" onClick={() => setThreadOpen((open) => !open)}>
          ...
          {isOwner ? (
            <>
              ...
              <Button type="button" variant="destructive" disabled={deleteCommentMutation.isPending} onClick={() => { void onDelete(); }}>
      ...
      {isEditing ? (
        <div className="mt-4 space-y-3">
          ...
          />
          <div className="flex flex-wrap gap-2">
            <Button type="button" disabled={updateCommentMutation.isPending} onClick={() => { void onSaveEdit(); }}>
        ...
        <div className="mt-5 space-y-4 border-t border-slate-200 pt-5">
          {isAuthenticated && canReply ? (
            <div className="space-y-3 rounded-2xl border border-slate-200 bg-slate-50/80 p-4">
              <p className="text-sm font-semibold text-slate-950">Reply to this comment</p>
              <textarea
                className="min-h-24 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none transition-colors focus:border-slate-400"
                value={replyContent}
              ...
              />
              <div className="flex flex-wrap gap-2">
                <Button type="button" disabled={replyToCommentMutation.isPending} onClick={() => { void onReply(); }}>
                  <Send className="size-4" />
                  {replyToCommentMutation.isPending ? "Posting..." : "Post reply"}
                </Button>
                <Button type="button" variant="outline" onClick={() => setThreadOpen(false)}>
                  Hide thread
                </Button>
              </div>
            </div>
          ...
          {repliesQuery.isPending ? (
            <LoadingState rows={2} title="Loading replies" description="Fetching this comment thread." />
            ...
            <EmptyState title="No replies yet" description="This thread does not have any replies yet." />
          ) : (
...
export function IdeaDetailClient({ ideaId, isAuthenticated, role }: IdeaDetailClientProps) {
  ...
  if (!ideaId) {
    return (
      <main className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        <EmptyState title="Idea not found" description="No idea identifier was provided in the route." />
      </main>
    );
  }
  ...
  if (ideaQuery.isPending) {
    return (
      <main className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        <LoadingState rows={8} title="Loading idea" description="Fetching the full idea record from the backend." />
      </main>
    );
  }
  ...
  if (!idea) {
    return (
      <main className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        <EmptyState title="Idea not found" description="The backend returned an empty idea payload." />
      </main>
    );
  }
  ...
  const bookmarkIdeaIds = bookmarks.map((bookmark) => getBookmarkIdeaId(bookmark));
  ...
  const visibleComments = topLevelComments.length > 0 ? topLevelComments : commentRecords;

  const voteBusy = voteIdeaMutation.isPending || updateVoteMutation.isPending || removeVoteMutation.isPending;
  const bookmarkBusy = bookmarkIdeaMutation.isPending || removeBookmarkMutation.isPending;
  ...
  const submitVote = async (nextVote: VoteType) => {
    ...
    if (!isAuthenticated) {
      setInteractionFeedback({ type: "error", text: "Log in before you vote on an idea." });
      return;
    }

    try {
      if (currentVote === nextVote) {
        const response = await removeVoteMutation.mutateAsync({ ideaId: idea.id });
        setInteractionFeedback({ type: "success", text: response.message || "Vote removed successfully." });
        return;
      }
      ...
      if (currentVote) {
        ...
        setInteractionFeedback({ type: "success", text: response.message || `Vote updated to ${nextVote}.` });
        return;
      }
      ...
      setInteractionFeedback({ type: "success", text: response.message || `${nextVote} vote submitted successfully.` });
    } catch (error) {
  ...
  const toggleBookmark = async () => {
    ...
    if (!isAuthenticated) {
      setInteractionFeedback({ type: "error", text: "Log in before you save ideas." });
      return;
    }

    try {
      if (isBookmarked) {
        const response = await removeBookmarkMutation.mutateAsync({ ideaId: idea.id });
        setInteractionFeedback({ type: "success", text: response.message || "Bookmark removed successfully." });
        return;
      }

      const response = await bookmarkIdeaMutation.mutateAsync({ ideaId: idea.id });
      setInteractionFeedback({ type: "success", text: response.message || "Idea bookmarked successfully." });
    } catch (error) {
      ...
      if (shouldAttemptRefetchFallback) {
        try {
          ...
          const refreshedIdeaIds = refreshedBookmarks.map((bookmark) => getBookmarkIdeaId(bookmark));
  ...
  const submitComment = async () => {
    ...
    if (!isAuthenticated) {
      setInteractionFeedback({ type: "error", text: "Log in before you comment." });
      return;
    }
    ...
    if (!content) {
      setInteractionFeedback({ type: "error", text: "Comment content cannot be empty." });
      return;
    }
    ...
    try {
      ...
      setCommentDraft("");
      setInteractionFeedback({ type: "success", text: response.message || "Comment posted successfully." });
    } catch (error) {
      setInteractionFeedback({
        type: "error",
        text: getInteractionErrorMessage(error, idea.status),
  ...
  return (
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Link href="/idea" className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition-colors hover:border-slate-300 hover:text-slate-950">
      ...
      <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-[0_20px_70px_-40px_rgba(15,23,42,0.35)]">
        <div className="grid gap-0 lg:grid-cols-[22rem_minmax(0,1fr)]">
          <div className="relative min-h-72 bg-[linear-gradient(135deg,#0f172a_0%,#1e293b_40%,#0ea5e9_100%)]">
            {hasText(idea.coverImageUrl) ? (
              <div className="absolute inset-0 bg-cover bg-center opacity-80" style={{ backgroundImage: `url(${idea.coverImageUrl})` }} />
            ) : null}
            <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(15,23,42,0.12),rgba(15,23,42,0.85))]" />
            <div className="relative flex h-full flex-col justify-end p-6 text-white">
              <p className="text-xs uppercase tracking-[0.22em] text-slate-200">{idea.category?.name ?? "Uncategorized"}</p>
              <h1 className="mt-3 text-3xl font-semibold leading-tight sm:text-4xl">{getIdeaTitle(idea)}</h1>
              <p className="mt-4 max-w-xl text-sm leading-7 text-slate-200">{getIdeaSummary(idea)}</p>
            </div>
          </div>
